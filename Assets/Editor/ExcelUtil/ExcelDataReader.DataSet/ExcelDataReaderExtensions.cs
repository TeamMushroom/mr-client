...
namespace ExcelDataReader
{
    ...
    {
        private static long NowMillisecond => DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond;
        ...
        /// <param name="completeAsDataTableAction">Invoke complete data as data table</param>
        /// <returns>A dataset with all workbook contents</returns>
        public static DataSet AsDataSet(this IExcelDataReader self, ExcelDataSetConfiguration configuration = null, Action<string, long> completeAsDataTableAction = null)
        {
            ...
            do
            {
                ...
                var beforeMillisecond = NowMillisecond;
                var table = AsDataTable(self, tableConfiguration);
                var afterMillisecond = NowMillisecond;
                completeAsDataTableAction?.Invoke(table.TableName, afterMillisecond - beforeMillisecond);
        ...
        public static string GetUniqueColumnName(DataTable table, string name)
        {
            var columnName = name;
            var i = 1;
        ...
        private static DataTable AsDataTable(IExcelDataReader self, ExcelDataTableConfiguration configuration)
        {
            ...
            var ignoreFirstNRow = configuration.IgnoreFirstNRow;
            var readRowCount = configuration.ReadRowCount;
            var readFieldCount = configuration.ReadFieldCount;
            ...
            while (self.Read())
            {
                if (first)
                {
                    ...
                    if (ignoreFirstNRow > 0)
                    {
                        ignoreFirstNRow--;
                        continue;
                    }

                    var fieldCount = self.FieldCount;
                    if (readFieldCount >= 0 && fieldCount > readFieldCount)
                    {
